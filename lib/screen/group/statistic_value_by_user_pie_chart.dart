...
class StatisticValueByUserPieChart extends StatelessWidget {
  ...
  StatisticValueByUserPieChart({
    ...
    if (statisticValueByUser.isEmpty) _totalValue = 0;
    else _totalValue = statisticValueByUser.map((statistic) => statistic.value).reduce((value, element) => value + element);
  }

  Widget _getBody(BuildContext context) {
    if (statisticValueByUser == null) {
      return Center(
        child: CircularProgressIndicator()
      );
    }

    if (statisticValueByUser.isEmpty) {
      return Center(
        child: Text(
